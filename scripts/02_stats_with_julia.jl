...
# load packages
using Distributions, DataFrames, Statistics, Plots
...
# create a sample data of size N
begin
	N = 1000
	univ_data = rand(1:1000, N)
	univ_data
end

# ╔═╡ c71912a9-c672-4eaf-b4e7-147a1aab88a1
# calculate basic statistics on your sample data
begin
	my_mean = mean(univ_data)
	my_median = median(univ_data)
	my_mode = mode(univ_data)
	
	my_var = var(univ_data)
	my_std = std(univ_data)
	my_range = maximum(univ_data) - minimum(univ_data)
	
	my_skewness = skewness(univ_data)
	my_kurtosis = kurtosis(univ_data)
end

# ╔═╡ e2157d5d-5322-4a55-a390-db2f03317528
begin
	# visualize your data
	histogram(univ_data, bins=20, label="data values", color=:grey)
	xlabel!("bins (buckets)")
	ylabel!("frequency")
end
...
begin
	binom_data = rand(Binomial(10, 0.3), 1000)
	histogram(binom_data)
end

# ╔═╡ ff0f698c-8a8b-4f6b-9b8e-b254ff5a3e4a
begin
	gamma_data = rand(Gamma(1, 1), 1000)
	histogram(gamma_data)
end
